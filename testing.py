...
def main():
    ...
    hwName = input('hwName:')
    useMains = True if input('main.cpp testing (m) or ref testing (r)?') == 'm' else False
...
def checkDirs(useMains):
    if (useMains == True) and (not os.path.isdir('./main')):
...
def countTests(useMains):
    out = None
    if (useMains == True):
        # counts # files in main/ (for main unit testing)
        out = subprocess.Popen("ls main| wc -l", stdout=subprocess.PIPE, stderr=subprocess.STDOUT , shell=True)
    else:
